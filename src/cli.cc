...
void wdisk(char *args, CommandLine *cli)
{
	...
	for (offset; cmp[offset] != '\0'; offset++)
	{
	}
	offset++;
...
void files(char *args, CommandLine *cli)
{
	...
	for (int i = 0; i < fileNum; i++)
	{
		...
		if (filterTags)
		{
			skip = cli->filesystem->GetTagFile(args, location, &tagNum) != location;
		}
...
void createFile(char *args, CommandLine *cli)
{
	...
	int i = 0;
	char fileName[33];
	for (i; i < strlen(argparse(args, 0)); i++)
	{
		fileName[i] = args[i];
	}
	fileName[i] = '\0';
	...
	{
		cli->PrintCommand("' already exists or couldn't be created for some reason.\n");
	}
}
...
void control(char *args, CommandLine *cli)
{
	...
	if (cli->network->activateServerCMD)
	{
		cli->PrintCommand("Command server mode activated.\n");
	}
	else
	{
		cli->PrintCommand("Command server mode deactivated.\n");
	}
}
...
void web(char *args, CommandLine *cli)
{
	...
	if (strcmp(argparse(args, 0), "start"))
	{
		...
		for (int i = 0; i < responseLength + messageLength; i++)
		{
			...
			if (i < responseLength)
			{
				cli->network->fileData[i] = RESPONSE_HTTP[i];
			}
			else
			{
				cli->network->fileData[i] = data[i - responseLength];
			}
		}
...
void terminal(char *args, CommandLine *cli)
{
	...
	if (cli->gui)
	{
		cli->appWindow->parent->CreateChild(APP_TYPE_TERMINAL, "Osaka's Terminal", 0);
	}
	else
	{
...
void kasugapaint(char *args, CommandLine *cli)
{
	...
	if (cli->gui)
	{
		cli->appWindow->parent->CreateChild(APP_TYPE_KASUGAPAINT, "KasugaPaint", 0);
	}
	else
	{
		cli->PrintCommand("This command is not available in text mode.\n");
...
void journal(char *args, CommandLine *cli)
{
	...
	if (cli->gui)
	{
		cli->appWindow->parent->CreateChild(APP_TYPE_JOURNAL, "Journal", 0);
	}
	else
	{
		cli->PrintCommand("This command is not available in text mode.\n");
...
void shinosaka(char *args, CommandLine *cli)
{
	...
	if (cli->gui)
	{
		cli->appWindow->parent->CreateChild(APP_TYPE_SHINOSAKA, "Shinosaka", 0);
	}
	else
	{
		cli->PrintCommand("This command is not available in text mode.\n");
...
void window(char *args, CommandLine *cli)
{
	...
	if (cli->gui && cli->userWindow == nullptr)
	{
		...
		if (strlen(args) < 2)
		{
			cli->userWindow = cli->appWindow->parent->CreateChild(0, "OSaka Window", cli);
		}
		else
		{
			cli->userWindow = cli->appWindow->parent->CreateChild(APP_TYPE_SCRIPT, args, cli);
		}
	...
	else
	{
		if (cli->gui)
		{
			cli->PrintCommand("Window is already active.\n");
		}
		else
		{
			cli->PrintCommand("This command is not available in text mode.\n");
...
void effect(char *args, CommandLine *cli)
{
	...
	if (strcmp(argparse(args, 0), "wave"))
	{
		...
		{
			cli->appWindow->parent->children[windowID]->Wave ^= 1;
		}
	}
	else if (strcmp(argparse(args, 0), "pixel"))
	{
		...
		{
			cli->appWindow->parent->children[windowID]->Pixelize ^= 1;
		}
	}
	else if (strcmp(argparse(args, 0), "rainbow"))
	{
		...
		{
			cli->appWindow->parent->children[windowID]->Rainbow ^= 1;
		}
	}
	else
	{
...
void cursor(char *args, CommandLine *cli)
{
	...
	for (int i = 0; i < cli->vga->gfxBufferSize; i++)
	{
		buf[i] = 0x00;
	}
	uint8_t *ptr = nullptr;
	...
	if (fileFound)
	{
		cli->appWindow->parent->LoadCursor(buf, w, h);
	}
	else
	{
		cli->appWindow->parent->LoadCursor(nullptr, w, h);
	}
}
...
void targetgui(char *args, CommandLine *cli)
{
	...
	if (cli->targetWindow)
	{
		cli->PrintCommand("Currently targeting user window.\n");
	}
	else
	{
		cli->PrintCommand("Currently targeting desktop buffer.\n");
	}
}
...
void putpixel(char *args, CommandLine *cli)
{
	...
	if (cli->targetWindow)
	{
		cli->userWindow->WritePixel(x, y, color);
	}
	else
	{
		cli->appWindow->parent->PutPixel(x, y, color);
	}
}
...
void drawrect(char *args, CommandLine *cli)
{
	...
	if (cli->targetWindow)
	{
		cli->userWindow->DrawRectangle(x0, y0, x1, y1, color, true);
	}
	else
	{
		cli->appWindow->parent->DrawRectangle(x0, y0, x1, y1, color, true);
	}
}
...
void drawline(char *args, CommandLine *cli)
{
	...
	if (cli->targetWindow)
	{
		cli->userWindow->DrawLine(x0, y0, x1, y1, color);
	}
	else
	{
		cli->appWindow->parent->DrawLine(x0, y0, x1, y1, color);
	}
}
...
void drawtext(char *args, CommandLine *cli)
{
	...
	if (cli->targetWindow)
	{
		cli->userWindow->PutText(args, x, y, c);
	}
	else
	{
		cli->appWindow->parent->PutText(args, x, y, c);
	}
}
...
void drawpic(char *args, CommandLine *cli)
{
	...
	if (cli->targetWindow)
	{
		cli->userWindow->FillBuffer(x, y, w, h, buf);
	}
	else
	{
		cli->appWindow->parent->FillBuffer(x, y, w, h, buf);
	}
}
...
void regPrint(char *args, CommandLine *cli)
{
	...
	cli->PrintCommand("EAX: ");
	cli->PrintCommand(int2str(cli->tm->eaxPrint));
	cli->PrintCommand("\nEBX: ");
	cli->PrintCommand(int2str(cli->tm->ebxPrint));
	cli->PrintCommand("\nECX: ");
	cli->PrintCommand(int2str(cli->tm->ecxPrint));
	cli->PrintCommand("\nEDX: ");
	cli->PrintCommand(int2str(cli->tm->edxPrint));

	cli->PrintCommand("\nESP: ");
	cli->PrintCommand(int2str(cli->tm->espPrint));
	cli->PrintCommand("\nEBP: ");
	cli->PrintCommand(int2str(cli->tm->ebpPrint));
	cli->PrintCommand("\n");
}
...
void help(char *args, CommandLine *cli)
{
	...
	else
	{
		...
		cli->PrintCommand("    ctrl+e - file edit mode (ctrl-r to write file)\n", WFFFFFF);
...
void azufetch(char *args, CommandLine *cli)
{
	...
	if (cli->gui)
	{
		w = WFFFFFF, p = WFF55FF, b = W5555FF, r = WFF5555;
	}
	else
	{
		w = WBABAFF, p = WBE00FF, b = W0000FF, r = WAA0055;
	}
	...
	cli->PrintCommand("  /------------\\   ", p);
	cli->PrintCommand("OS:  ", b);
	cli->PrintCommand("osakaOS\n", w, TEXT_BOLD);
	cli->PrintCommand(" /---________---\\  ", p);
	cli->PrintCommand("CPU: ", r);
	cli->PrintCommand("486 (harcoded btw)\n", w, TEXT_UNDERLINE);
	cli->PrintCommand("/---/-v-/\\-v-\\---\\ ", p);
	cli->PrintCommand("MEM: ", b);
	cli->PrintCommand(int2str(memory), w);
	cli->PrintCommand("MB of memory\n", w);
	cli->PrintCommand("|--/-/ v  v \\-\\--| ", p);
	cli->PrintCommand("VID: ", r);
	if (cli->gui == false)
	{
		cli->PrintCommand("80x25 Textmode\n", w);
	}
	else if (cli->vga->vesa)
	{
		cli->PrintCommand("640x480 VESA 101h\n", w);
	}
	else
	{
		cli->PrintCommand("320x200 Mode 13h\n", w);
	}
...
void CommandLine::CleanCommandLine()
{
	...
	if (returnVar->typeStr != nullptr)
	{
		mm->free(returnVar->typeStr);
	}
...
void CommandLine::PrintCommand(char *str, uint16_t color, uint8_t flags)
{
	...
	if (color)
	{
		...
		if (gui)
		{
			this->appWindow->textColor = setTextColor(true, color);
		}
		else
		{
			setTextColor(true, color);
		}
	...
	if (gui)
	{
		...
		{
			this->userWindow->Print(str, flags);
		}
		else
		{
			this->appWindow->Print(str, flags);
		}
	}
	else
	{
...
char *CommandLine::command(char *cmd, uint8_t length)
{
	...
	if (this->conditionIf == false && result != hash("fi") && result != hash("end"))
	{
